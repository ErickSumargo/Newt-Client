...
public class ChallengeListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (Session.with(context).getUserType().equals(Constant.STUDENT)) {
            if (challenge.getAttempt() == -1 || (challenge.getAttempt() > 0 && challenge.getStatus() == 0)) {
                ...
                if (challenge.getStatus() == 1) {
                    ...
                    itemHolder.icStatus.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_correct));
                    itemHolder.icStatus.setVisibility(View.VISIBLE);
                } else {
                    itemHolder.detailCont.setBackgroundColor(context.getResources().getColor(R.color.colorFalse));
                    itemHolder.answer.setVisibility(View.GONE);

                    itemHolder.icStatus.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_incorrect));
                    itemHolder.icStatus.setVisibility(View.VISIBLE);
                }
            }
        } else {
            itemHolder.detailCont.setBackgroundColor(context.getResources().getColor(R.color.graySemiLight));
            itemHolder.answer.setText("RINCIAN");
            itemHolder.answer.setVisibility(View.VISIBLE);
            itemHolder.icStatus.setVisibility(View.GONE);
        }
        ...
        itemHolder.item.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                ((ChallengeActivity) context).startActivityForResult(intent, 0);
            }
        });
    }
